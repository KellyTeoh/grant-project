...
Grant claim review system that auto-analyzes actuals from SharePoint, classifies invoice eligibility against grant rules, flags exceptions, and generates ready-to-approve claim packages — reducing manual prep by 70%.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "296506091+KellyTeoh@users.noreply.github.com"
  git config user.name "KellyTeoh"
  ```
